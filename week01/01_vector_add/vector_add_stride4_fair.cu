...
__global__ void vector_add_stride4(const float *a,
                                   const float *b,
                                   float *c,
                                   int n)
{
...
int main(int argc, char **argv)
{
    ...
    size_t input_bytes = (4 * N) * sizeof(float);
    size_t output_bytes = N * sizeof(float);

    std::vector<float> h_a(4 * N, 1.0f);
    std::vector<float> h_b(4 * N, 2.0f);
    ...
    // Warm-up
    vector_add_stride4<<<blocks, threads>>>(d_a, d_b, d_c, N);
